...
def load_data(dataset_name="achrafothman/aslg_pc12"):
    try:
        ...
        try:
            # If not found, try loading as local CSV file
            data = load_dataset('csv', data_files=dataset_name)["train"]
            print(f"Loaded dataset from local CSV file: {dataset_name}")
        except Exception as e:
            raise ValueError(f"Failed to load dataset from both Hugging Face Hub and local path: {dataset_name}") from e
...
def preprocess_examples(batch, tokenizer, max_source_length, max_target_length):
    source = ["translate ASL to English: " + str(gloss) for gloss in batch["gloss"]]
    target = [str(item) for item in batch["text"]]
